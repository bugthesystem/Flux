...
const TEST_PATH: &str = "/tmp/kaos-ipc-trace-bench";
...
fn run_ipc_trace_bench() -> (f64, bool) {
    ...
    let publisher = thread::spawn(move || {
        let mut pub_handle = Publisher::<Slot8>::new(TEST_PATH, 64 * 1024).unwrap();
    ...
    let subscriber = thread::spawn(move || {
        let mut sub_handle = Subscriber::<Slot8>::new(TEST_PATH).unwrap();
