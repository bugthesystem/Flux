//! # flux-rudp
//! 
//! High-performance reliable UDP transport built on flux ring buffers.
...
use flux::disruptor::{MessageRingBuffer, RingBufferConfig, RingBufferEntry};
use flux::crc32;
...
pub const FLAG_NO_CRC: u8 = 0x01;      // Skip CRC for maximum speed
...
/// This is 3x smaller than ReliableUdpHeader for maximum throughput
#[repr(C, packed)]
...
impl FastHeader {
    ...
    pub fn actual_length(&self) -> usize {
        (self.frame_length & !FAST_HEADER_MAGIC) as usize
    }
}
...
impl ReliableUdpHeader {
    ...
    pub fn new(session_id: u32, sequence: u64, msg_type: MessageType, payload_len: u16) -> Self {
        let timestamp = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis() as u32;
    ...
    pub fn from_packet_zero_copy(packet: &[u8]) -> Option<(&Self, &[u8])> {
...
impl ReliableUdpRingBufferTransport {
    ...
    pub fn auto(config: ReliableUdpConfig) -> std::io::Result<Self> {
        let bind_addr: std::net::SocketAddr = config.local_addr
            .parse()
            .map_err(|e|
                std::io::Error::new(
        ...
        let remote_addr: std::net::SocketAddr = config.remote_addr
            .parse()
            .map_err(|e|
                std::io::Error::new(
                    std::io::ErrorKind::InvalidInput,
    ...
    /// High-performance batch send (24-byte header, no CRC)
    #[inline]
    pub fn send_batch_fast(&mut self, data: &[&[u8]]) -> std::io::Result<usize> {
        let batch_size = data.len();
        if batch_size == 0 {
            return Ok(0);
        }
        ...
        if let Some((slot_seq, _slots)) = self.send_window.try_claim_slots(batch_size) {
            ...
            SEND_BUFFER.with(|buf_cell| {
                ...
                let estimated_size = actual * (4 + ReliableUdpHeader::SIZE + 64);
                let current_cap = buf.capacity();
                if current_cap < estimated_size {
                    buf.reserve(estimated_size - current_cap);
                }
                ...
                for i in 0..actual {
                    let seq = slot_seq + (i as u64);
                    let msg = data[i];
                    
                    let packet_size = (ReliableUdpHeader::SIZE + msg.len()) as u32;
                    buf.extend_from_slice(&packet_size.to_le_bytes());
                    
                    let header = ReliableUdpHeader {
                        session_id: 0,
                        sequence: seq,
                        msg_type: MessageType::Data as u8,
                        flags: FLAG_NO_CRC | FLAG_BATCH,
                        payload_len: msg.len() as u16,
                        timestamp: 0,
                        checksum: 0,
                    };
    ...
    /// Original send_batch with CRC (for reliable mode)
    pub fn send_batch_reliable(&mut self, data: &[&[u8]]) -> std::io::Result<usize> {
        let batch_size = data.len();
        if batch_size == 0 {
            return Ok(0);
        }
        ...
        if let Some((slot_seq, slots)) = self.send_window.try_claim_slots(batch_size) {
            ...
            SEND_BUFFER.with(|buf_cell| {
                let mut packed_packet = buf_cell.borrow_mut();
                packed_packet.clear();
                ...
                for i in 0..actual {
                    ...
                    let packet_size = (ReliableUdpHeader::SIZE + msg.len()) as u32;
                    packed_packet.extend_from_slice(&packet_size.to_le_bytes());
                    
                    let mut header = ReliableUdpHeader {
                        session_id: 0,
                        sequence: seq,
                        msg_type: MessageType::Data as u8,
                        flags: FLAG_BATCH,
                    ...
                    packed_packet.extend_from_slice(unsafe {
                        std::slice::from_raw_parts(&header as *const _ as *const u8, ReliableUdpHeader::SIZE)
                    });
                    packed_packet.extend_from_slice(msg);
                }

                let mut offset = 0;
                for i in 0..actual {
                    let packet_size = u32::from_le_bytes(packed_packet[offset..offset+4].try_into().unwrap()) as usize;
                    let packet_data = &packed_packet[offset+4..offset+4+packet_size];
                    slots[i].set_sequence(slot_seq + (i as u64));
                    slots[i].set_data(packet_data);
                    offset += 4 + packet_size;
                }

                if let Err(_e) = socket.send_to(&packed_packet, remote_addr) {
                    #[cfg(feature = "debug")] eprintln!("[WARN] Failed to send batch: {}", _e);
                }
            });
    ...
    pub fn send_nak(&self, missing_seq: u64) {
        ...
        #[cfg(feature = "debug")] eprintln!("[NAK-SEND] Sending single NAK for seq {} to {}", missing_seq, self.remote_nak_addr);
        if let Err(e) = self.nak_socket.send_to(&packet, self.remote_nak_addr) {
            ...
            #[cfg(feature = "debug")] eprintln!("[NAK-SEND-OK] Single NAK sent successfully");
        }
    }
    ...
    pub fn get_next_send_seq(&self) -> u64 {
        self.next_send_seq
    }

    #[cfg(target_os = "linux")]
    pub fn send_many(&mut self, data: &[&[u8]]) -> std::io::Result<usize> {
        use std::os::unix::io::AsRawFd;
        
        let batch_size = data.len().min(64);
        if batch_size == 0 {
            return Ok(0);
        }

        if let Some((slot_seq, slots)) = self.send_window.try_claim_slots(batch_size) {
            let actual = std::cmp::min(slots.len(), batch_size);
            let mut packets: Vec<Vec<u8>> = Vec::with_capacity(actual);

            for i in 0..actual {
                let seq = slot_seq + (i as u64);
                let msg = data[i];
                let mut header = ReliableUdpHeader::new(0, seq, MessageType::Data, msg.len() as u16);
                header.calculate_checksum(msg);
            ...
            let sent = unsafe {
                self.batch_sender.send_batch(socket_fd, &packet_refs, &self.remote_addr)?
            };
    ...
    #[cfg(target_os = "linux")]
    pub fn receive_many(&mut self, max_count: usize) -> std::io::Result<Vec<Box<[u8; 2048]>>> {
        use std::os::unix::io::AsRawFd;
        
        let socket_fd = self.socket.as_raw_fd();
        let count = unsafe { self.batch_receiver.recv_batch(socket_fd)? };
        
        let mut results = Vec::with_capacity(count);
        
        for i in 0..count {
            if let Some(packet) = self.batch_receiver.get_packet(i) {
                if packet.len() >= ReliableUdpHeader::SIZE {
                    if let Some((header, payload)) = ReliableUdpHeader::from_packet_with_payload_check(packet) {
                        let msg_type = header.msg_type;
                        if msg_type == (MessageType::Data as u8) && header.verify_checksum(payload) {
                            let seq = header.sequence;
                            self.recv_window.insert(seq, payload);
                        }
                    }
    ...
    pub fn receive_batch(&mut self, max_count: usize) -> Vec<Box<[u8; 2048]>> {
        ...
        RECV_BUFFERS.with(|bufs_cell| {
            ...
            if data.len() > 4 {
                ...
                while offset + 4 <= data.len() {
                    ...
                    if offset + pkt_len <= data.len() {
                        let packet_data = &data[offset..offset + pkt_len];

                        if pkt_len >= ReliableUdpHeader::SIZE {
                            if
                                let Some(header) = ReliableUdpHeader::from_bytes(
                                    &packet_data[..ReliableUdpHeader::SIZE]
                                )
                            {
                                let seq = header.sequence;
            ...
            } else if len >= ReliableUdpHeader::SIZE {
                if
                    let Some(header) = ReliableUdpHeader::from_bytes(
