//! sendmmsg/recvmmsg support for high-performance batch UDP I/O
//!
...
impl BatchReceiver {
    ...
    pub fn get_packet(&self, index: usize) -> Option<&[u8]> {
        if index >= self.msgvec.len() {
            return None;
        }

        let len = self.msgvec[index].msg_len as usize;
        if len == 0 {
            return None;
        }

        Some(&self.buffers[index][..len])
    }
}
...
impl BatchReceiver {
    ...
    pub fn get_packet(&self, _index: usize) -> Option<&[u8]> {
        None
    }
}
