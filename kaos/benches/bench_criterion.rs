//! Criterion-based kaos benchmark
//!
...
/// Batch API benchmark
fn kaos_batch<T: RingBufferEntry + Copy + Default + Send + Sync + 'static>(events: u64) -> u64 {
...
fn kaos_per_event<T: RingBufferEntry + Copy + Default + Send + Sync + 'static>(events: u64) -> u64 {
    let ring = Arc::new(RingBuffer::<T>::new(RING_SIZE).unwrap());
    let producer_cursor = ring.producer_cursor();
...
fn benchmark_slot_sizes_batch(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("kaos", "8B (Slot8)"), |b| {
        b.iter(|| kaos_batch::<Slot8>(TOTAL_EVENTS))
    });

    group.bench_function(BenchmarkId::new("kaos", "16B (Slot16)"), |b| {
        b.iter(|| kaos_batch::<Slot16>(TOTAL_EVENTS))
    });

    group.bench_function(BenchmarkId::new("kaos", "32B (Slot32)"), |b| {
        b.iter(|| kaos_batch::<Slot32>(TOTAL_EVENTS))
    });

    group.bench_function(BenchmarkId::new("kaos", "64B (Slot64)"), |b| {
        b.iter(|| kaos_batch::<Slot64>(TOTAL_EVENTS))
    });

    group.bench_function(BenchmarkId::new("kaos", "128B (MessageSlot)"), |b| {
        b.iter(|| kaos_batch::<MessageSlot>(TOTAL_EVENTS))
    });
...
fn benchmark_batch_vs_per_event(c: &mut Criterion) {
    ...
    group.bench_function("per-event (1/call)", |b| {
        b.iter(|| kaos_per_event::<Slot8>(TOTAL_EVENTS))
    });
