//! SPMC ring buffer - single producer, multiple consumers.

use std::sync::atomic::{ AtomicU64, Ordering };
use std::sync::Arc;

use crate::disruptor::completion_tracker::{
    CompletionTracker,
    ReadGuard,
    BatchReadGuard,
    ReadableRing,
...
impl<T: RingBufferEntry> SpmcRingBuffer<T> {
    ...
    /// Try to claim slots. Returns next cursor if space available.
...
mod tests {
    ...
    fn test_spmc_early_exit_no_data_loss() {
        ...
        for val in [1, 2, 3] {
            ...
            unsafe {
                ring.write_slot(cursor, Slot8 { value: val });
            }
            ring.publish(next);
            cursor = next;
        }
    ...
    fn test_spmc_out_of_order_completion() {
        ...
        for i in 0..3 {
            let next = ring.try_claim(1, cursor).unwrap();
            unsafe {
                ring.write_slot(cursor, Slot8 { value: i });
            }
            ring.publish(next);
            cursor = next;
        }
