...
//!
//! ```rust,no_run
//! use kaos::disruptor::SharedRingBuffer;
//! use kaos::disruptor::Slot8;
//!
//! // Process A - Create and write
//! let mut rb = SharedRingBuffer::<Slot8>::create("/tmp/kaos-shared", 1024)?;
//! rb.try_send(&[1, 2, 3, 4])?;
//!
//! // Process B - Open and read
//! let mut rb = SharedRingBuffer::<Slot8>::open("/tmp/kaos-shared")?;
//! rb.receive(|data| {
//!     // Process the Slot8 data
...
/// Magic number to identify kaos shared ring buffer files
        ...
        if header.magic != MAGIC {
            ...
            return Err(
                io::Error::new(
                    ...
                    "Invalid kaos shared ring buffer (wrong magic)"
                )
            );
        }
