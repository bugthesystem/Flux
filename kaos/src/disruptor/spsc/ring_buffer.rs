...
//! ## APIs
//!
//! For batch operations, use slice-based access:
...
impl<T: RingBufferEntry> RingBuffer<T> {
    ...
    ///
    /// Returns slice of slots for reading.
...
mod tests {
    ...
    fn test_heap_allocation() {
        ...
        assert!(ring.producer_cursor().load(Ordering::Relaxed) == 0);
    }
    ...
    fn test_mapped_write_read() {
        let mut ring = RingBuffer::<Slot8>::new_mapped(1024).unwrap();

        let cursor = 0u64;
        if let Some(next) = ring.try_claim(2, cursor) {
            unsafe {
                ring.write_slot(0, Slot8 { value: 42 });
                ring.write_slot(1, Slot8 { value: 43 });
            }
            ring.publish(next);
