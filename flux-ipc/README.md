...
Shared memory IPC using flux ring buffers.
...
```rust
use flux_ipc::{Publisher, SmallSlot};

let mut publisher = Publisher::<SmallSlot>::new("/tmp/channel", 64 * 1024)?;
...
```rust
use flux_ipc::{Subscriber, SmallSlot};

let mut subscriber = Subscriber::<SmallSlot>::new("/tmp/channel")?;
