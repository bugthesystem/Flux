...
/// Test SPSC ordering with SmallSlot (8 bytes)
#[test]
fn test_spsc_ordering_small_slot() {
    let ring = Arc::new(RingBuffer::<SmallSlot>::new(64 * 1024).unwrap());
                ...
                for slot in slots {
                    if slot.value != expected {
                        ...
                        eprintln!(
                            ...
                            cons_cursor + read, expected, slot.value
                        );
    ...
    println!("\n=== SPSC Ordering Test (SmallSlot) ===");
    println!("Written: {}", written);
    println!("Read: {}", read);
    println!("Errors: {}", error_count);
    println!("Rate: {:.2} M/s", written as f64 / TEST_DURATION_SECS as f64 / 1_000_000.0);
    ...
    assert!(written - read <= 1, "Producer/consumer mismatch: {} written, {} read", written, read);
}

/// Stress test with high contention (small ring)
#[test]
fn test_high_contention_ordering() {
    let ring = Arc::new(RingBuffer::<SmallSlot>::new(1024).unwrap());
    let running = Arc::new(AtomicBool::new(true));
    let errors = Arc::new(AtomicU64::new(0));
    let producer_seq = Arc::new(AtomicU64::new(0));
...
fn test_sequence_wraparound() {
    let ring = Arc::new(RingBuffer::<SmallSlot>::new(256).unwrap());
