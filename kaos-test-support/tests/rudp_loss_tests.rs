...
//!
//! Tests that kaos-rudp correctly handles packet loss scenarios.

use kaos_test_support::loss::{DropDecision, LossGenerator, LossPattern};
...
fn test_no_loss_baseline() {
    ...
    let receiver =
        thread::spawn(move || simulated_receiver(&receiver_socket, &checker_clone, running_clone));
...
fn test_random_loss_statistics() {
    ...
    assert!(
        (avg_loss - target_loss).abs() < 0.01,
        "Loss rate {} not close to target {}",
        avg_loss,
        target_loss
    );
}
...
fn test_combined_loss_patterns() {
    ...
    println!(
        "Combined loss dropped {} packets: {:?}",
        dropped.len(),
        &dropped[..dropped.len().min(15)]
    );
