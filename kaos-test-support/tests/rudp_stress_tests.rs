...
use kaos_test_support::stress::{print_summary, StressConfig, StressCounters, StressRunner};
...
fn test_udp_baseline_stress() {
    let config = StressConfig::new(5).with_batch_size(100);
...
fn test_udp_sequence_stress() {
    ...
    println!(
        "Gaps: {} (total missing: {})",
        stats.gap_count, stats.total_missing
    );
...
fn test_udp_burst_stress() {
    ...
    assert!(
        total_received as f64 / total_sent as f64 > 0.9,
