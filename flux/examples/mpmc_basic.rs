...
use flux::disruptor::{MpmcRingBuffer, SmallSlot};
use std::sync::Arc;
...
fn main() {
    ...
    let ring_buffer = Arc::new(MpmcRingBuffer::<SmallSlot>::new(RING_SIZE).unwrap());
            ...
            for number in start_num..end_num {
                loop {
                    if let Some(sequence) = ring_buffer_clone.try_claim(1) {
                        unsafe {
                            ring_buffer_clone.write_slot(sequence, SmallSlot { value: number });
                        }
    ...
    for _ in 0..NUM_CONSUMERS {
        loop {
            if let Some(sequence) = ring_buffer.try_claim(1) {
                unsafe {
                    ring_buffer.write_slot(sequence, SmallSlot { value: 0 });
                }
