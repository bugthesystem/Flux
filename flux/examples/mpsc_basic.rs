...
use flux::disruptor::{MpscProducerBuilder, MpscConsumerBuilder, MpscRingBuffer, SmallSlot, MpscEventHandler};
use std::sync::Arc;
use std::thread;
...
fn main() {
    ...
    let ring_buffer = Arc::new(MpscRingBuffer::<SmallSlot>::new(RING_SIZE).unwrap());
    ...
    let consumer_thread = thread::spawn(move || {
        ...
        impl MpscEventHandler<SmallSlot> for AverageHandler {
            fn on_event(&mut self, event: &SmallSlot, _seq: u64, _end_of_batch: bool) {
    ...
    if count == total_sent && sum == expected_sum {
        ...
        println!("  🚀 Clean API - Beautiful DevX!");
    } else {
