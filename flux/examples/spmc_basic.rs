...
use flux::disruptor::{SpmcRingBuffer, SmallSlot};
use std::sync::Arc;
...
fn main() {
    ...
    let ring_buffer = Arc::new(SpmcRingBuffer::<SmallSlot>::new(RING_SIZE).unwrap());
    ...
    let producer_thread = thread::spawn(move || {
        ...
        for number in 1..=MAX_NUMBER {
            loop {
                if let Some(next) = ring_buffer.try_claim(1, producer_cursor) {
                    unsafe {
                        ring_buffer.write_slot(producer_cursor, SmallSlot { value: number });
                    }
        ...
        for _ in 0..NUM_CONSUMERS {
            loop {
                if let Some(next) = ring_buffer.try_claim(1, producer_cursor) {
                    unsafe {
                        ring_buffer.write_slot(producer_cursor, SmallSlot { value: 0 });
                    }
                    ring_buffer.publish(next);
                    producer_cursor = next;
                    break;
