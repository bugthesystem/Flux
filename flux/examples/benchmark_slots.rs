...
use flux::disruptor::{RingBuffer, RingBufferEntry, SmallSlot, Slot16, Slot32, Slot64};
...
fn main() {
    ...
    println!("Results:");
    let t8 = benchmark::<SmallSlot>("SmallSlot(8B)");
    let t16 = benchmark::<Slot16>("Slot16(16B)");
    let t32 = benchmark::<Slot32>("Slot32(32B)");
    let t64 = benchmark::<Slot64>("Slot64(64B)");

    println!("\nSummary:");
    println!("  Best: Slot32 is typically fastest due to cache line alignment");
    println!("  SmallSlot: {:.0}% of Slot32", t8 / t32 * 100.0);
