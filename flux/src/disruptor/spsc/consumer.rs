//! High-level Consumer API with excellent DevX
//!
...
/// High-performance consumer for MessageRingBuffer
pub struct Consumer {
...
/// Macro for easy event handler creation
#[macro_export]
macro_rules! event_handler {
    ($name:ident, $body:expr) => {
        struct $name;
        impl $crate::disruptor::consumer::EventHandler for $name {
            fn on_event(&mut self, event: &$crate::disruptor::MessageSlot, sequence: u64, end_of_batch: bool) {
                $body(event, sequence, end_of_batch)
            }
        }
    };
}
