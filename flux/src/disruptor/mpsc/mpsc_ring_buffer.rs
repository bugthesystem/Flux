...
impl<T: RingBufferEntry> MpscRingBuffer<T> {
    pub fn new(size: usize) -> Result<Self> {
        if size < 64 {
            return Err(FluxError::config("MPSC ring buffer must be at least 64 slots"));
        }
    ...
    #[inline(always)]
    pub fn is_published(&self, sequence: u64) -> bool {
        let (avail_idx, bit_idx) = self.calculate_indices(sequence);
        let expected_flag = self.calculate_flag(sequence);
        let availability = self.available[avail_idx].load(Ordering::Acquire);
        let bit = (availability >> bit_idx) & 1;
        bit != expected_flag
    }
