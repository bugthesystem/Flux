...
impl<T: RingBufferEntry> MpscProducer<T> {
    ...
    /// Publish batch - Maximum performance!
    #[inline(always)]
    ...
    /// Try to publish batch - returns actual published count
    #[inline(always)]
    pub fn try_publish_batch<F>(&self, max_count: usize, mut writer: F) -> usize
    where
        F: FnMut(usize, &mut T),
    {
        if let Some(start_seq) = self.ring_buffer.try_claim(max_count) {
