...
impl CompletionTracker {
    ...
    pub fn claim_cursor(&self) -> u64 {
        self.claim_cursor.0.load(Ordering::Relaxed)
    }

    /// Get count of in-flight operations.
    #[inline(always)]
    pub fn in_flight_count(&self) -> u64 {
        let claim = self.claim_cursor.0.load(Ordering::Relaxed);
        let completed = self.completed_cursor.0.load(Ordering::Relaxed);
        claim.saturating_sub(completed)
    }
}
...
mod tests {
    ...
    fn test_fast_path() {
        ...
        assert_eq!(tracker.claim_cursor(), 100);
    }
}
