...
mod tests {
    ...
    fn test_mpmc_basic_read_commit() {
        let ring = MpmcRingBuffer::<SmallSlot>::new(1024).unwrap();

        // Producer writes
        let seq = ring.try_claim(1).unwrap();
        unsafe { ring.write_slot(seq, SmallSlot { value: 42 }); }
        ring.publish(seq);
    ...
    fn test_mpmc_early_exit_no_data_loss() {
        let ring = Arc::new(MpmcRingBuffer::<SmallSlot>::new(1024).unwrap());

        // Producer writes values 1, 2, 3
        for val in [1, 2, 3] {
            let seq = ring.try_claim(1).unwrap();
            unsafe { ring.write_slot(seq, SmallSlot { value: val }); }
            ring.publish(seq);
        }
                ...
                for val in start..end {
                    loop {
                        if let Some(seq) = ring_clone.try_claim(1) {
                            unsafe { ring_clone.write_slot(seq, SmallSlot { value: val }); }
        ...
        for _ in 0..num_consumers {
            loop {
                if let Some(seq) = ring.try_claim(1) {
                    unsafe { ring.write_slot(seq, SmallSlot { value: 0 }); }
                    ring.publish(seq);
                    break;
                }
                std::hint::spin_loop();
