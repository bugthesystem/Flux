...
use flux::disruptor::{ RingBuffer, RingBufferEntry, SmallSlot, Slot32, Slot64, MessageSlot };
...
fn bench_smallslot_heap() -> u64 {
    let result = run_observability_bench::<SmallSlot>(false);
...
fn bench_smallslot_mapped() -> u64 {
    let result = run_observability_bench::<SmallSlot>(true);
    assert!(result.verified, "Data integrity check failed!");
    TOTAL_EVENTS
}
